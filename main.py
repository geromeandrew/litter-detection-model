    ...
    with open(os.path.join(config.data_dir, 'train.txt')) as f:
        for file_name in f.readlines():
            image_path = os.path.join(
                config.data_dir, config.image_dir, file_name.rstrip() + '.jpg')
            label_path = os.path.join(
                config.data_dir, config.label_dir, file_name.rstrip() + '.xml')
            if os.path.exists(image_path) and os.path.exists(label_path):
                if os.path.exists(os.path.join(config.data_dir, 'TF')):
                    file_names.append(os.path.join(
                        config.data_dir, 'TF', file_name.rstrip() + '.tf'))
                else:
        ...
        for step, inputs in enumerate(dataset):
            ...
            if step % steps == 0:
                model.save_weights(os.path.join(
                    "weights", f"model_{config.version}.h5"))
...
def test():
    def draw_bbox(image, boxes):
        for box in boxes:
            ...
            c1, c2 = (coordinate[0], coordinate[1]
    ...
    def test_fn():
        ...
        for file_name in tqdm.tqdm(file_names):
            image = cv2.imread(os.path.join(
                config.data_dir, config.image_dir, file_name + '.jpg'))
        ...
        # loss, accuracy = model.predict(test_images[0][numpy.newaxis, ...])
        # print(f'LOSS: {loss}')
...
class AnchorGenerator:
    ...
    def iou(self, boxes, clusters):  # 1 box -> k clusters
        ...
        cluster_w_matrix = numpy.reshape(
            numpy.tile(clusters[:, 0], (1, n)), (n, k))
        min_w_matrix = numpy.minimum(cluster_w_matrix, box_w_matrix)

        box_h_matrix = numpy.reshape(boxes[:, 1].repeat(k), (n, k))
        cluster_h_matrix = numpy.reshape(
            numpy.tile(clusters[:, 1], (1, n)), (n, k))
    ...
    def generator(self, boxes, k, dist=numpy.median):
        ...
        clusters = boxes[numpy.random.choice(
    ...
    def get_boxes():
        ...
        file_names = [file_name[:-4]
                      for file_name in os.listdir(os.path.join(config.data_dir, config.label_dir))]
